...
const StyledNavigationBarBrandContainer = styled(Navbar.Brand).attrs(usePropsWithConfig)`
  display: flex;
...
const NavigationBarBrand = () => {
  ...
  return (
    <StyledNavigationBarBrandContainer onClick={useScrollTo(config.ids.hero)} href={"#"}>
      <Brand />
    </StyledNavigationBarBrandContainer>
  )
}

const StyledNavigationBarToggle = styled(Navbar.Toggle).attrs(usePropsWithConfig)``;

const StyledCollapsableNavigation = styled(Navbar.Collapse).attrs(usePropsWithConfig)`
...
const NavigationLink = ({destination}: {destination: {id: string, name: string}}) => {
  return (
    <StyledNavigationLink href={`#${destination.id}`} onClick={useScrollTo(destination.id)} active={false}>{destination.name}</StyledNavigationLink>
  )
}
      ...
      <StyledNavigationBar expand="sm" fixed="top">
        <StyledNavigationBarContainer fluid>
          ...
          <StyledCollapsableNavigation id={config.ids.navigation}>
            <StyledNavigation>
              {config.nav.destinations.map((destination, index) => <NavigationLink key={index} destination={destination} />)}
            </StyledNavigation>
